...
interface NavItem { label: string; to: string; icon: React.ComponentType<{ className?: string }>; perm: Permission | Permission[] }
  ...
  return (
    <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar">
      <div className="flex items-center gap-2 px-5 h-16 border-b border-sidebar-border">
        {logo ? (
          <img src={logo} alt={`${brandName} logo`} className="h-9 w-9 rounded-xl object-cover ring-1 ring-border" />
        ) : (
        ...
        <div className="flex flex-col leading-tight">
          ...
          <span className="text-[10px] uppercase tracking-wider text-muted-foreground">School · NGO · Ops</span>
        </div>
      </div>
              ...
              <div className="space-y-0.5">
                {visible.map(item => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={cn("h-4 w-4 shrink-0", active ? "text-brand-accent" : "text-muted-foreground group-hover:text-foreground")} />
      ...
      <div className="border-t border-sidebar-border p-3">
        <Link to="/settings" className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm text-sidebar-foreground hover:bg-sidebar-accent">
